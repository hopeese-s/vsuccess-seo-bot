                ...
                try {
                    ...
                    for (const event of events) {
                        if (event.type === 'message' && event.message.type === 'text') {
                            ...
                            if (text === 'โพสต์คิวถัดไป') {
                            ...
                            } else if (text.startsWith('เพิ่มคิวภาค:')) {
                                const regionsMap = {
                                    'ภาคเหนือ': ['เชียงใหม่', 'เชียงราย', 'ลำปาง', 'ลำพูน', 'แม่ฮ่องสอน', 'น่าน', 'พะเยา', 'แพร่', 'อุตรดิตถ์'],
                                    'ภาคใต้': ['ภูเก็ต', 'สงขลา', 'สุราษฎร์ธานี', 'นครศรีธรรมราช', 'กระบี่', 'พังงา', 'ตรัง', 'พัทลุง', 'ปัตตานี', 'ยะลา', 'นราธิวาส', 'ชุมพร', 'ระนอง', 'สตูล'],
                                    'ภาคกลาง': ['กำแพงเพชร', 'ชัยนาท', 'นครนายก', 'นครสวรรค์', 'พระนครศรีอยุธยา', 'พิจิตร', 'พิษณุโลก', 'เพชรบูรณ์', 'ลพบุรี', 'สมุทรสงคราม', 'สระบุรี', 'สิงห์บุรี', 'สุโขทัย', 'สุพรรณบุรี', 'อ่างทอง', 'อุทัยธานี'],
                                    ...
                                    'ภาคอีสาน': ['นครราชสีมา', 'ขอนแก่น', 'อุบลราชธานี', 'อุดรธานี', 'บุรีรัมย์', 'ศรีสะเกษ', 'สุรินทร์', 'ร้อยเอ็ด', 'สกลนคร', 'ชัยภูมิ', 'เลย', 'หนองคาย', 'มหาสารคาม', 'กาฬสินธุ์', 'นครพนม', 'มุกดาหาร', 'ยโสธร', 'อำนาจเจริญ', 'หนองบัวลำภู', 'บึงกาฬ'],
                                ...
                                let matchRegion = null;
                                let baseKeyword = '';
                                
                                const content = text.replace('เพิ่มคิวภาค:', '');
                                for (const region of Object.keys(regionsMap)) {
                                    if (content.includes(region)) {
                                        matchRegion = region;
                                        baseKeyword = content.replace(region, '').trim();
                                        break;
                                    }
                                }

                                if (!matchRegion || !baseKeyword) {
                                    await replyToLine(replyToken, '❌ รูปแบบผิด กรุณาพิมพ์เช่น:\nเพิ่มคิวภาค: สายคล้องคอ ภาคเหนือ\nเพิ่มคิวภาค: รับทำบัตรพนักงาน ภาคใต้', LINE_CHANNEL_ACCESS_TOKEN);
                                } else {
                                    const provinces = regionsMap[matchRegion];
                                    const kwList = provinces.map(prov => `${baseKeyword} ${prov}`);
                                    const count = await addKeywordsToQueue(kwList);
                                    await replyToLine(replyToken, `✅ เพิ่ม ${count} คีย์เวิร์ดสำหรับ "${matchRegion}" ลงในระบบเรียบร้อยแล้ว`, LINE_CHANNEL_ACCESS_TOKEN);
                                }
